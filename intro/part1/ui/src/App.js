...
import { MsgImpl__factory } from './ethers-contracts';
import {
  createNonce,
...
//  const { connect, disconnect, signerAddress } = useEthereumProvider();
  ...
  const sendClickHandler = useCallback(() => {
    ...
    (async () => {
      ...
      const sendMsg = MsgImpl__factory.connect(
      ...
      const sendTx = await sendMsg.sendStr(new Uint8Array(Buffer.from(SentText)), nonce);
...
//        '0xC89Ce4735882C9F0f0FE26686c53074E09B0D550'  //TBD!
  ...
  const receiveClickHandler = () => {document.getElementById(rtaId).value = fetchCb();}

  return (
    <li style={{backgroundColor: clr, marginTop:10, padding: 0, borderRadius: 10}}>
      <p style={{marginTop:0, textAlign:'left'}}> {c}</p>
      <button className='chain-row-button' onClick={sendClickHandler}>Send</button> 
      <textarea id={staId} className='chain-row-text-area' placeholder='message to send' defaultValue='eth test'/>
      <button className='chain-row-button' onClick={receiveClickHandler}>Receive</button> 
...
function Chain ({c, sendCB, fetchCb}) {
  ...
  console.log('Chain render '+ c);
  const sendClickHandler = () => {sendCB(document.getElementById(staId).value)}
  const receiveClickHandler = () => {document.getElementById(rtaId).value = fetchCb();}
  ...
  return (
    <li style={{backgroundColor: clr, marginTop:10, padding: 0, borderRadius: 10}}>
      <p style={{marginTop:0, textAlign:'left'}}> {c}</p>
      <button className='chain-row-button' onClick={sendClickHandler}>Send</button> 
      <textarea id={staId} className='chain-row-text-area' placeholder='message to send' defaultValue={sendText}/>
      <button className='chain-row-button' onClick={receiveClickHandler}>Receive</button> 
      <textarea id={rtaId} className='chain-row-text-area' readOnly/>
    </li>
  )
...
//  const lastSentCbEther = useCallback((txt) => {setLastSent(txt); setLastSentSource("Ether")}, []);
  const lastSentCbEther = ((txt) => {setLastSent(txt); setLastSentSource("Ether")});
  const lastSentCbSolana = ((txt) => {setLastSent(txt); setLastSentSource("Solana")});
  const lastSentCbTerra = ((txt) => {setLastSent(txt); setLastSentSource("Terra")});

  const lastFetchCbEther = (() => {return lastSent});
  const lastFetchCbSolana = (() => {return lastSent});
  const lastFetchCbTerra = (() => {return lastSent});

  return (
    <div className="App">
      <header className="App-header">
        <p style={{fontSize: 'calc(10px + 2vmin)'}}>Post messages via Wormhole</p>
        <div>Last message sent: <br /><pre><mark>{lastSent}</mark></pre>from {lastSentSource}</div>
        <ul style={{listStyleType: 'none', margin: 0, padding:0, width: '90%'}}>
          <EthChain c='Etherium' sendCB={lastSentCbEther} fetchCb={lastFetchCbEther}></EthChain>
