...
impl VAA {
    ...
    pub fn digest(&self) -> Option<[u8; 32]> {
        ...
        // We hash twice such that the hash of the message is the main item that is being verified
        // by contracts. If we don't do this, we would have to pass in the full message to the
        // signing methods when verifying on chain. This means all consuming applications should be
        // aware that they should work with the hash of the message being sent rather than the
        // message itself in order to reduce computation/data usage.
        let body: [u8; 32] = {
